...
if __name__ == '__main__':
    ...
    pfail = 0
    get_opto = 1
    root_folder_name = 'om_choice_nopfail_opto'              # another glm0h
    ...
    if not get_opto:
        ...
        # clean data
        data_batch12 = om.loc[
            (om.mouse_id < 13) & (om.lick_side_freq != -2) & (om.prev_choice != -2) & (om.prev_reward_prob == 0.5) & (
                        om.prev_choice2 != -2)]
        data_batch34 = om.loc[
    ...
    if choice_or_accuracy == 'acc':
        ...
        if pfail:
            formula = 'lick_side_freq ~ -1 + freq_trans + C(prev_failure) + z_freq_trans:C(prev_failure) + prev_choice'
            formula_unnormalized = 'lick_side_freq ~ -1 + freq_trans + C(prev_failure) + freq_trans:C(prev_failure) + prev_choice'
        else:
            formula = 'lick_side_freq ~ -1 + freq_trans + prev_choice'
    ...
    for mouse_index in range(len(animal_list)):
        ...
        om_tmp = om_cleaned_session.loc[om_cleaned_session['mouse_id'] == animal_list[mouse_index]].copy().reset_index(drop=True)
        T = len(om_tmp)
